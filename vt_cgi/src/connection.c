...
int check_login(connection_t *con)
{
	char cmd[512] = {0};
	...
	if (temp)
		*temp = '\0';
	snprintf(cmd, sizeof(cmd), "ubus call session list \"{\\\"ubus_rpc_session\\\":\\\"%s\\\"}\" 2>/dev/null", sess);
	char sess_out[1024];
	int recv = shell_printf(cmd, sess_out, sizeof(sess_out));
	if (recv)
		return 1;
	else {
		return 0;
