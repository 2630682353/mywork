...
char * http_post(const char *url, const char *dev_mac, const char *post_str){  
    ...
    while((n=http_tcpclient_recv(socket_fd,lptemp)) > 0){  
        memcpy(lpbuf+current, lptemp, n);
		if (test_if_close(lpbuf)) {
        	break;
        }
        current+=n; 
    }  
...
//  printf("发送请求:\n%s\n",lpbuf);  
    ...
    //printf(http_parse_result(lpbuf));
    http_tcpclient_close(socket_fd);  
...
/*
main() {
	char *res = http_get("http://www.wiair.cc/protocol.csp?fname=system&opt=main&function=get&login=1");
	printf("%s\n", res);
}*/